...
6. **Cross-copy consistency** — where a document is deliberately mirrored, the body content is byte-identical; only frontmatter/navigation differ. For CLAUDE.md this is enforced mechanically, not by review: `00 Governance/CLAUDE.md` is the canonical source and the root `CLAUDE.md` is generated from it. Verify with `./scripts/sync-claude-md.sh --check` (macOS/Linux/Git Bash) or `.\scripts\sync-claude-md.ps1 -Check` (Windows PowerShell); never hand-edit the root file to resolve a mismatch.
