...
- The FA-02 migration-cycle drill goes red and [[Database Engineer]] has established it is a genuine downgrade defect rather than a migration-design problem. That skill leads first and hands the established defect here; this skill leads the root-cause trace from that point.
- The FA-03 backup/restore drill goes red and its cause is not established. Nothing pre-classifies this one — no skill owns a handback for it, and it runs `if: always()`, so it can go red for a genuine restore defect, a `pg_dump`/client-version mismatch, or a runner condition. It fires this skill directly, and check 4 decides where the cause actually belongs. Both drills leave their output in the `api-drill-output` artifact.
