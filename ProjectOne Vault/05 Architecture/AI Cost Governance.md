...
- **No budget is configured by default**, so a new workspace is unmetered rather than blocked. Refusing every call on a platform that has not asked anyone to set a limit would be the wrong default; whether it stays that way in production is a launch decision ([[STEP-25 Foundation Audit and Internal Readiness]]).
