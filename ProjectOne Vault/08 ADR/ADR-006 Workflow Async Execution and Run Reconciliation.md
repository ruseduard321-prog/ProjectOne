...
- **I24.** **No mismatched definition executes or derives workflow state.** Before execution, recovery **and** approval, the definition this deployment would use must match the run's recorded `workflow_type` and `definition_version`. On every path a mismatch fails closed with a fixed public-safe message naming no version: **no provider is called, no step is admitted, and no approval or claim is consumed**, and the run's persisted steps, outputs and history remain available for review.

  **The effect on the run's *status* differs by path, and the invariant does not claim otherwise:**
  - **Approval and recovery** detect the mismatch in the route, before the protected command is invoked, so nothing is written and the run stays in the state it was already in.
  - **A worker** is already holding a job. The refusal is terminal, the job dead-letters, and **D5 reconciles the linked non-terminal run to `failed` in the same statement** — unchanged, and deliberately given no exception, because a dead-lettered job against a live run is exactly the stranded pair D5 exists to prevent.

  What happens to an incompatible run afterwards is a product decision — migrate it, restart it, abandon it — rather than a runtime one.
...
- **P53.** **An approval is audited.** `app_approve_workflow_step` writes exactly one `workflow.approved` row naming the workspace, `auth.uid()`, the run, the step index and the created job id, with `created_at` as the approval time. No lease or claim token appears in it.
- **P54.** **A refused approval writes nothing.** A member's refusal and an approval named at the wrong step each leave the audit table exactly as they found it. Four concurrent approvals leave one grant, one job and one audit row.
- **P55.** **The history survives consumption.** After admission clears `approved_by`, the `workflow.approved` row and its actor are still readable. This is the property §Column Necessity relies on when it declines an `approved_at` column.
- **P56.** **No observer sees every step complete under a live run.** With the settling transaction held open, a second connection sees neither write; after commit it sees both. The contradictory state is unobservable at every instant.
- **P57.** **A failed claimed step cannot be re-entered across the boundary.** An execution attempting admission while the settlement is uncommitted blocks on the locks the settlement holds, and after commit is refused by the terminal run. It never admits.
- **P58.** **A failed run transition rolls its step settlement back.** Driven through the real runner against a real database: the step row the pair would have written does not exist, while an unrelated intermediate step committed earlier is untouched.
- **P59.** **The lock order is uniform.** Every command takes run, then step, then job — asserted against the installed function bodies, not the migration source.
- **P60.** **A redelivery after completion is a success.** It calls no provider, changes no workflow state, settles `succeeded`, and is never dead-lettered.
- **P61.** **The worker refuses a run whose version moved, and every effect is asserted.** Zero provider calls; zero `workflow_step_runs` rows; the job `dead_lettered`; and the run `failed` by D5's reconciliation — **not** left in its previous state. The run stays readable through its route.
- **P62.** **Recovery refuses before reading `requires_approval`.** Refused in the route before the command runs, so the failed run is left exactly as it was; 422 with the fixed message.
- **P63.** **Approval refuses rather than enqueueing work the worker will reject.** `approved_by` stays null and no job is created.
- **P64.** **A matching version is unaffected.** The ordinary path is unchanged, asserted on a completed run and one provider call.
- **P65.** **Both new child foreign keys lead a covering index**, and the index is not narrowed by `status` — terminal linked rows are exactly the ones referential maintenance must still find. The whole-schema advisor check runs alongside it against a recorded baseline of pre-existing findings.
