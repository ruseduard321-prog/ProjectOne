...
- **Continuous integration:** GitHub Actions runs `.github/workflows/ci.yml` on every push and pull request as of STEP-06 — a `web` job (lint, type-check, test, build) and an `api` job (lint, format, type-check, test). Both are green. The workflow is the authority on what must pass before a merge; run the same commands locally first. **Run status and per-step results are readable** from this environment via `WebFetch` against the unauthenticated GitHub REST API (`/actions/runs`, `/actions/runs/{id}/jobs`) — enough to identify the failing job and step by name. **Step logs return 403 and remain owner-only.** See Known Gaps.
...
- **GitHub workflow results are not readable from this environment.** Commits push successfully and the [[MCP/GitHub|GitHub MCP]] reads repository data (validated against the real repository as of STEP-06), but the MCP exposes no workflow-run tool, `gh` is not installed, and the repository is private so unauthenticated API and browser access return 404. Confirming a CI run is therefore an owner action. Installing `gh` or adding a workflow-run capability would close this.
	- **Largely closed during STEP-19**, in two parts:
		- **Run status and per-step results *are* readable** via `WebFetch` against the unauthenticated GitHub REST API — `/repos/{owner}/{repo}/actions/runs` and `/actions/runs/{id}/jobs` both answer, despite the repository being private in the web UI. That is enough to identify the failing job and the exact failing step by name, which is how run #28 was diagnosed as a container failure rather than a test failure. **Step *logs* (`/logs`) still return 403** and remain owner-only.
		- The full CI suite now also runs locally against a real PostgreSQL (see above), so a red run is reproducible here.
	- **Order of operations for a red run:** read `/actions/runs/{id}/jobs` first to find which step failed. If it is `Test`, reproduce locally. If it is anything earlier, the suite never ran and the cause is environmental.
