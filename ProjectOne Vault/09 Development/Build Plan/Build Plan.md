...
**The credential mismatch that blocked STEP-17 was root-caused and resolved.** The role is created without a password by design (`d7b95c1f4e08` — a credential in a migration is a credential in source control), so the pairing between the role's password and `REQUEST_DATABASE_URL` has **no committed source of truth and no automated consistency check**. That is correct on the security merits and is exactly why a mismatch stays invisible until the first request that touches a tenant table. [[DOC-02 Validate the Request-Path Credential at Startup]] records the fix; it is unscheduled.

**Claude's execution environment cannot reach the development database, and the architecture was deliberately left unchanged.** `db.<project-ref>.supabase.co` publishes an AAAA record and no A record, while that environment has no IPv6 route — so connections fail at DNS resolution, before authentication. The project is healthy and the owner's environment is unaffected. Supabase documents direct connections as IPv6-by-default with a paid IPv4 add-on, and the session pooler as the IPv4 alternative. **The owner's decision on 2026-08-03: treat this as an execution-environment limitation, keep the direct connection architecture, and hand live verification to the owner** rather than reshape the project's connection model around one sandbox's missing route.

**Cross-tenant isolation is verified structurally but not behaviourally at head.** The database-backed tests skip without `PROJECTONE_TEST_DATABASE_URL`, which now includes STEP-17's 18 `provider_credentials` isolation tests. Migration `f1a4c8d29b57` has been verified structurally and by revision chain, but **has never been applied to a live database** — see [[STEP-17 AI Router and Provider Abstraction#Live verification handed to the owner]] for the two commands that close this.
