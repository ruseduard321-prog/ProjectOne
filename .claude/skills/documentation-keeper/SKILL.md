...
6. **Cross-copy consistency** — deliberately mirrored documents are body-identical; only frontmatter/navigation differ. CLAUDE.md is enforced mechanically: `00 Governance/CLAUDE.md` is canonical, root `CLAUDE.md` is generated. Verify with `./scripts/sync-claude-md.sh --check` or `.\scripts\sync-claude-md.ps1 -Check`; never hand-edit the root file.
